...
{
    public class ObstacleSerializedEditor : SerializedEditorAbstract
    {
        public ObstacleSerializedEditor(HierarchySimButton hierarchySimButton, Panel inspectorPanel)
        {
            name = "Obstacle Serialized";
        ...
        public override void removeComponentButton_Click(object sender, EventArgs e, string name)
        {
            EditorEventListenSystem.eventManager.SendEvent(new OnEditorFunction {
                editorFunction = () => {
                    simButton.simObject.entity.RemoveComponent<ObstacleComp>();
                    simButton.simObject.objectData.RemoveSerializedComp(typeof(ObstacleSerialized));
                }
            });
