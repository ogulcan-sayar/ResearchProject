...
{
    public class TargetSerializedEditor : SerializedEditorAbstract
    {
        public TargetSerializedEditor(HierarchySimButton hierarchySimButton, Panel inspectorPanel)
        {
            name = "Target Serialized";
        ...
        public override void removeComponentButton_Click(object sender, EventArgs e, string name)
        {
            EditorEventListenSystem.eventManager.SendEvent(new OnEditorFunction {
                editorFunction = () => {
                    simButton.simObject.entity.RemoveComponent<TargetComp>();
                    simButton.simObject.objectData.RemoveSerializedComp(typeof(TargetSerialized));
                }
            });
